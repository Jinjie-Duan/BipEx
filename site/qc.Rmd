...
dt_loc$Total = rowSums(dt_loc[,c("Controls", "Bipolar Total")])
...
dt_sex_removal <- rbindlist(
    list(dt[1,],
        list(c("Samples with sex swap", "Samples after sex swap removal"),
             ...
             c(round(n_sex_removed[1] / unlist(dt[1,2]) * 100,1),
               round(n_after_sex_removed$Samples / unlist(dt[1,2]) * 100,1)))), use.names=FALSE)
...
dt_ibd_removal <- rbindlist(
    list(dt[1,],
        list(c("Related samples for removal", "Samples after IBD removal"),
             ...
             c(round(n_ibd_removed[1] / unlist(dt[1,2]) * 100,1),
               round(n_after_ibd_removed$Samples / unlist(dt[1,2]) * 100,1)))), use.names=FALSE)
...
names(dt_pca) <- gsub("phenotype.", "", names(dt_pca))
dt_pca_case <- dt_pca %>% filter(PHENOTYPE_COARSE == "Bipolar Disorder")
dt_pca_control <- dt_pca %>% filter(PHENOTYPE_COARSE == "Control")
