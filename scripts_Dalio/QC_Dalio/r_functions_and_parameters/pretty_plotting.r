...
create_pretty_forest <- function(df, title, save_figure=FALSE, file='file_out',
    width=160, height=90, scaling=1, y_label='', pass_order=NULL, print_p=TRUE, horizontal=TRUE, hline_at=0)
{
...
create_pretty_hist <- function(df, aest, x_label, threshold=NULL, threshold_max=NULL, file='file_out',
    title='', binwidth=0.002, width=160, height=90, scaling=1, save_figure=FALSE, xlim=NULL, key_label='')
{
    p <- ggplot(df, aest)
...
create_pretty_boxplots <- function(df, aes, aes_col, threshold=NULL,
    ...
    outlier.shape=NA, n_ticks=10)
{
    p = ggplot(df, aes) +
        geom_boxplot(outlier.shape=outlier.shape, coef=0, color='grey50', fill='grey95', show.legend=FALSE) + 
        geom_jitter(width=0.2, height=0, size=jitter_size, aes_col, show.legend=legend, alpha=0.2, stroke=0.05) + 
...
create_pretty_scatter <- function(df, aes, file='file_out', save_figure=FALSE, 
...
{
    p <- create_pretty_scatter(df, aes, file=file, save_figure=save_figure,
