...
create_pretty_hist(df, aes(x=qc.call_rate), threshold=T_variant_call_rate, x_label='Call Rate', xlim=c(0.9,1), save_figure=save_figures,
	file=paste0(PLOTS, '15_variant_call_rate'))
...
if(save_figures) {
	ggsave(paste0(PLOTS,'15_call_rate_cdf.jpg'), p, width=160, height=90, units='mm')
	ggsave(paste0(PLOTS,'15_call_rate_cdf.pdf'), p, width=160, height=90, units='mm')
}
...
create_pretty_hist(df, aes(x=call_rate_diff), x_label='Call Rate Difference',
    ...
    xlim=c(-(1-T_variant_call_rate),(1-T_variant_call_rate)), save_figure=save_figures, file=paste0(PLOTS, '15_call_rate_diff'))

create_pretty_cumulative(df, aes(x=qc.p_value_hwe), x_label='p(HWE)',
    threshold=T_pHWE, xlim=c(0,1), save_figure=save_figures, file=paste0(PLOTS, '15_pHWE'))
