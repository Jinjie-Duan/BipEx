...
# ellipse.1 <- ellipse(0, 0.14, -0.08, 0.02, 0.02)
# ellipse.2 <- ellipse(0, -0.055, -0.045, 0.02, 0.02)
ellipse.1 <- ellipse(0, 0.125, 0.10, 0.02, 0.02)
ellipse.2 <- ellipse(0, 0.05, 0.055, 0.02, 0.02)

lines(ellipse.1[,1], ellipse.1[,2], lwd=2, col='indianred3')
lines(ellipse.2[,1], ellipse.2[,2], lwd=2, col='indianred3')

in_ell_1 <- in_ellipse(df_PCs_USA$PC1, df_PCs_USA$PC2, 0, 0.125, 0.10, 0.02, 0.02)
in_ell_2 <- in_ellipse(df_PCs_USA$PC1, df_PCs_USA$PC2, 0, 0.05, 0.055, 0.02, 0.02)

AJ <- 2 * in_ell_1 + in_ell_2
# Create labellings
df_PCs_USA$AJ <- factor(AJ, labels=c("not AJ", "half AJ", "AJ"))

jpeg(paste0(PLOTS, "12_AJ_in_US_PC1_PC2.jpg"), width=192, height=144, units='mm', res=300, quality=100)
plot(df_PCs_USA$PC1, df_PCs_USA$PC2, xlab="Principal Component 1", ylab="Principal Component 2", col=c('indianred3', 'cornflowerblue', 'lightgreen')[df_PCs_USA$AJ])
dev.off()
...
jpeg(paste0(PLOTS, "12_AJ_remove_in_US_PC1_PC2.jpg"), width=192, height=144, units='mm', res=300, quality=100)
df_PCs_USA$remove <- above_below_line(df_PCs_USA$PC1, df_PCs_USA$PC2, m1, c1, "above") | above_below_line(df_PCs_USA$PC1, df_PCs_USA$PC2, m2, c2, "below")
plot(df_PCs_USA$PC1, df_PCs_USA$PC2, xlab="Principal Component 1", ylab="Principal Component 2", col=c('indianred3', 'cornflowerblue')[as.factor(df_PCs_USA$remove)])
...
df_out_mainland_EUR <- df_1kg %>% filter(s %in% EUR_strict) %>% filter(!(Location %in% c("Karolinska", "Umea"))) %>% select(s)
df_out_SWE <- df_1kg %>% filter(s %in% EUR_strict) %>% filter(Location %in% c("Karolinska", "Umea")) %>% select(s)
...
df_AJ_EUR$case_control[where_AJ] <- as.character(df_AJ_EUR$AJ[where_AJ])
...
AJ_SAMPLES <- '../../samples_Dalio/12_AJ.sample_list'
HALF_AJ_SAMPLES <- '../../samples_Dalio/12_half_AJ.sample_list'
