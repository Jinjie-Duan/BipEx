...
df_pheno <- fread("../../phenotype_data/BIP_phenotype_information_cleaned_new_subtype_information_added_and_harmonised_final.tsv") %>% rename(s=SAMPLE_ALIAS)
...
df_initial_summary_count <- data.table(
	...
	Samples = c(nrow(df),
				...
				nrow(filter(df_merged, PHENOTYPE_COARSE_NEW=='Unknown')),
				nrow(df_merged[which(df_merged$s %in% low_coverage),])),
	"Bipolar cases" = c(nrow(df_merged %>% filter(PHENOTYPE_COARSE_NEW == "Bipolar Disorder")),
				NA,
				NA,
				nrow(df_merged[which((df_merged$s %in% low_coverage) & df_merged$PHENOTYPE_COARSE_NEW == "Bipolar Disorder"),])),
	"Controls" = c(nrow(df_merged %>% filter(PHENOTYPE_COARSE_NEW == "Control")),
				NA,
				NA,
				nrow(df_merged[which((df_merged$s %in% low_coverage) & df_merged$PHENOTYPE_COARSE_NEW == "Control"),])))

df_merged <- df_merged[-which(df_merged$s %in% low_coverage),] %>% filter(PHENOTYPE_COARSE_NEW != 'Unknown')
df_initial_summary_count <- rbindlist(list(df_initial_summary_count, list("Samples after initial filter", nrow(df_merged),
	nrow(df_merged %>% filter(PHENOTYPE_COARSE_NEW == "Bipolar Disorder")),
	nrow(df_merged %>% filter(PHENOTYPE_COARSE_NEW == "Control")))), use.names=FALSE)
...
df_summary_count <- data.table(
	...
	Samples = c(nrow(df),
				...
				nrow(df_out)),
	"Bipolar Cases" = c(nrow(df %>% filter(PHENOTYPE_COARSE_NEW == "Bipolar Disorder")),
			    nrow(filter(df, call_rate <= T_sample_callRate) %>% filter(PHENOTYPE_COARSE_NEW == "Bipolar Disorder")),
				nrow(filter(df, PCT_CONTAMINATION >= T_pct_contamination) %>% filter(PHENOTYPE_COARSE_NEW == "Bipolar Disorder")),
				nrow(filter(df, PCT_CHIMERAS >= T_pct_chimeras) %>% filter(PHENOTYPE_COARSE_NEW == "Bipolar Disorder")),
				nrow(filter(df, dp_stats.mean <= T_dpMean) %>% filter(PHENOTYPE_COARSE_NEW == "Bipolar Disorder")),
				nrow(filter(df, gq_stats.mean <= T_gqMean) %>% filter(PHENOTYPE_COARSE_NEW == "Bipolar Disorder")),
				length(which(df_out$s %in% (df %>% filter(PHENOTYPE_COARSE_NEW == "Bipolar Disorder"))$s))),
	Controls = c(nrow(df %>% filter(PHENOTYPE_COARSE_NEW == "Control")),
			    nrow(filter(df, call_rate <= T_sample_callRate) %>% filter(PHENOTYPE_COARSE_NEW == "Control")),
				nrow(filter(df, PCT_CONTAMINATION >= T_pct_contamination) %>% filter(PHENOTYPE_COARSE_NEW == "Control")),
				nrow(filter(df, PCT_CHIMERAS >= T_pct_chimeras) %>% filter(PHENOTYPE_COARSE_NEW == "Control")),
				nrow(filter(df, dp_stats.mean <= T_dpMean) %>% filter(PHENOTYPE_COARSE_NEW == "Control")),
				nrow(filter(df, gq_stats.mean <= T_gqMean) %>% filter(PHENOTYPE_COARSE_NEW == "Control")),
				length(which(df_out$s %in% (df %>% filter(PHENOTYPE_COARSE_NEW == "Control"))$s))))
