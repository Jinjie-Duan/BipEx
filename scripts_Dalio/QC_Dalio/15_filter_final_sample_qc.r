...
dt_pheno <- fread("../../phenotype_data/BIP_phenotype_information_cleaned_new_subtype_information_added_and_harmonised.tsv") %>% rename(s=SAMPLE_ALIAS) %>% select(s, PHENOTYPE_COARSE_NEW)
if(any(duplicated(dt_pheno$s))) {
  dt_pheno <- dt_pheno[-which(duplicated(dt_pheno$s)),]
}

df_after_merged <- merge(df_after, dt_pheno, by='s')
...
# r_ti_tv
df_keep_ti_tv <- group_by(df_after_merged, phenotype.PROJECT_OR_COHORT) %>%
  summarise(mean=mean(sample_qc.r_ti_tv), sd=sd(sample_qc.r_ti_tv)) %>%
  inner_join(df_after_merged, by='phenotype.PROJECT_OR_COHORT') %>%
...
# r_het_hom_var
df_keep_het_hom_var <- group_by(df_after_merged, phenotype.PROJECT_OR_COHORT) %>%
...
df_keep_insertion_deletion <- group_by(df_after_merged, phenotype.PROJECT_OR_COHORT) %>%
...
# n_singletons
df_keep_n_singletons <- group_by(df_after_merged, phenotype.LOCATION) %>%
  summarise(mean=mean(sample_qc.n_singleton), sd=sd(sample_qc.n_singleton)) %>%
  inner_join(df_after_merged, by='phenotype.LOCATION') %>%
...
df_final_sample_summary <- data.table(Filter = c("Samples after population filters",
                     ...
                     Samples = c(nrow(df_after_merged),
                                ...
                                nrow(df_keep)),
                     'Bipolar Cases' = c(nrow(df_after_merged %>% filter(PHENOTYPE_COARSE_NEW=="Bipolar Disorder")),
                      nrow(df_after_merged %>% filter(PHENOTYPE_COARSE_NEW=="Bipolar Disorder")) - nrow(df_keep_ti_tv %>% filter(PHENOTYPE_COARSE_NEW == "Bipolar Disorder")),
                      nrow(df_after_merged %>% filter(PHENOTYPE_COARSE_NEW=="Bipolar Disorder")) - nrow(df_keep_het_hom_var %>% filter(PHENOTYPE_COARSE_NEW == "Bipolar Disorder")),
                      nrow(df_after_merged %>% filter(PHENOTYPE_COARSE_NEW=="Bipolar Disorder")) - nrow(df_keep_insertion_deletion %>% filter(PHENOTYPE_COARSE_NEW == "Bipolar Disorder")),
                      nrow(df_after_merged %>% filter(PHENOTYPE_COARSE_NEW=="Bipolar Disorder")) - nrow(df_keep_n_singletons %>% filter(PHENOTYPE_COARSE_NEW == "Bipolar Disorder")),
                      nrow(df_keep %>% filter(PHENOTYPE_COARSE_NEW.x=="Bipolar Disorder"))),
                     Controls = c(nrow(df_after_merged %>% filter(PHENOTYPE_COARSE_NEW=="Control")),
                      nrow(df_after_merged %>% filter(PHENOTYPE_COARSE_NEW=="Control")) - nrow(df_keep_ti_tv %>% filter(PHENOTYPE_COARSE_NEW == "Control")),
                      nrow(df_after_merged %>% filter(PHENOTYPE_COARSE_NEW=="Control")) - nrow(df_keep_het_hom_var %>% filter(PHENOTYPE_COARSE_NEW == "Control")),
                      nrow(df_after_merged %>% filter(PHENOTYPE_COARSE_NEW=="Control")) - nrow(df_keep_insertion_deletion %>% filter(PHENOTYPE_COARSE_NEW == "Control")),
                      nrow(df_after_merged %>% filter(PHENOTYPE_COARSE_NEW=="Control")) - nrow(df_keep_n_singletons %>% filter(PHENOTYPE_COARSE_NEW == "Control")),
                      nrow(df_keep %>% filter(PHENOTYPE_COARSE_NEW.x=="Control"))))
